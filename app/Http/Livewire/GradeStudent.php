...
class GradeStudent extends Component
{
    ...
    public function updatedAcakanKe($acakan_ke)
    {
        if (!empty($acakan_ke)) {
            ...
            $this->resultData = $this->perhitunganMSFSelfSiswa($acakan_ke);
        } else {
            $this->toggleCard = false;
        }
    }

    protected function perhitunganMSFSelfSiswa($acakan_ke)
    {
        $data = $this->perhitunganMSF($acakan_ke);

        $idLogin = Auth::id();
        $dataSelfIDArray = array_search($idLogin, array_column($data, 'user_id'));
        $dataSelf = $data[$dataSelfIDArray];
